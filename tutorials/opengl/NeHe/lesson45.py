...
def IsExtensionSupported(TargetExtension):
    ...
    if (found_extension == False):
        gl_supports_extension = False
        print
        "OpenGL rendering context does not support '%s'" % (TargetExtension)
        return False
    ...
    try:
        ...
        print
        "PyOpenGL supports '%s'" % (TargetExtension)
    except:
        traceback.print_exc()
        print
        'Failed to import', extension_module_name
        print
        "OpenGL rendering context supports '%s'" % (TargetExtension),
        return False
...
def InitGL(Width, Height):  # We call this right after our OpenGL window is created.
    ...
    if (g_fVBOSupported):
        ...
        if (not glInitVertexBufferObjectARB()):
            print
            "Help!  No GL_ARB_vertex_buffer_object"
            sys.exit(1)
            return False
...
def DrawGLScene():
    ...
    if (g_fVBOSupported):
        ...
        glTexCoordPointer(2, GL_FLOAT, 0,
                          g_pMesh.m_pTexCoords_as_string);  # // Set The Vertex Pointer To Our TexCoord Data
